...
from dto import CNPJFieldNest, CNPJFieldInterface, ImpactLevel, Status

class NestAnalyzer(BaseAnalyzer):
    """Analisador específico para projetos NestJS"""
    
    def __init__(self):
        super().__init__("nest_bff", CNPJFieldNest)
    ...
    def find_cnpj_fields(self, files: List[Dict]) -> List[CNPJFieldInterface]:
        """Encontra campos relacionados a CNPJ nos arquivos NestJS"""
        cnpj_fields = []
            ...
            for line_num, line in enumerate(lines, 1):
                for pattern in self.cnpj_patterns['field_names']:
                    if re.search(pattern, line, re.IGNORECASE):
                        field = self._analyze_nest_cnpj_field(
    ...
    def _analyze_nest_cnpj_field(self, file_path: str, line_num: int, line: str, full_content: str) -> Optional[CNPJFieldInterface]:
        """Analisa um campo CNPJ específico em arquivos NestJS"""
        ...
        # Determinar tipo e tamanho do campo para NestJS
        field_type, field_size = self._extract_field_type_and_size(line, file_path)
        
        # Determinar impacto e status
        impact_level, status, action_needed = self._assess_nest_impact(field_type, field_size)
        
        # Extrair informações específicas para NestJS
        nest_type = self._extract_nest_type(file_path, line)
        decorator_type = self._extract_decorator_type(line)
        validation_decorators = self._extract_validation_decorators(line)
    ...
    def _assess_nest_impact(self, field_type: str, field_size: Optional[int]) -> tuple:
        """Avalia o impacto da mudança do CNPJ alfanumérico para NestJS"""
        if field_type in ['NUMBER', 'INTEGER']:
            return ImpactLevel.HIGH, Status.INCOMPATIBLE, "Alterar tipo para string"
        elif field_type == 'STRING':
            if field_size and field_size < 14:
                return ImpactLevel.CRITICAL, Status.INCOMPATIBLE, f"CRÍTICO: Tamanho {field_size} < 14. Alterar para string sem limite ou com limite >= 18"
            elif field_size and field_size < 18:
        ...
        elif field_type in ['CNPJ_VALIDATION', 'DOCUMENT_VALIDATION']:
            return ImpactLevel.HIGH, Status.NEEDS_ANALYSIS, "Validação precisa ser atualizada para CNPJ alfanumérico"
        elif field_type in ['REQUIRED_FIELD', 'OPTIONAL_FIELD']:
            return ImpactLevel.MEDIUM, Status.ATTENTION, "Verificar se precisa de validação específica"
        elif field_type in ['ASYNC_METHOD', 'SERVICE_CLASS', 'CONTROLLER_CLASS']:
            return ImpactLevel.MEDIUM, Status.NEEDS_ANALYSIS, "Verificar implementação do método"
        else:
            return ImpactLevel.MEDIUM, Status.NEEDS_ANALYSIS, "Análise manual necessária"

    def _extract_nest_type(self, file_path: str, line: str) -> Optional[str]:
        """Extrai o tipo NestJS baseado no arquivo e linha"""
        file_path_lower = file_path.lower()
        line_lower = line.lower()
        
        if 'controller' in file_path_lower or '@controller' in line_lower:
            return 'NESTJS_CONTROLLER'
        elif 'service' in file_path_lower or '@injectable' in line_lower:
            return 'NESTJS_SERVICE'
        elif 'dto' in file_path_lower or 'dto' in line_lower:
            return 'NESTJS_DTO'
        elif 'entity' in file_path_lower or '@entity' in line_lower:
            return 'NESTJS_ENTITY'
        elif 'interface' in file_path_lower or 'interface' in line_lower:
            return 'NESTJS_INTERFACE'
        elif 'module' in file_path_lower or '@module' in line_lower:
            return 'NESTJS_MODULE'
        elif 'guard' in file_path_lower or '@useguards' in line_lower:
            return 'NESTJS_GUARD'
        elif 'interceptor' in file_path_lower or '@useinterceptors' in line_lower:
            return 'NESTJS_INTERCEPTOR'
        elif 'pipe' in file_path_lower or '@usepipes' in line_lower:
            return 'NESTJS_PIPE'
        else:
            return 'NESTJS_GENERIC'

    def _extract_decorator_type(self, line: str) -> Optional[str]:
        """Extrai o tipo de decorator da linha"""
        decorators = [
            '@Controller', '@Injectable', '@Entity', '@Column', '@IsString',
            '@IsNumber', '@IsBoolean', '@IsArray', '@IsObject', '@IsNotEmpty',
            '@IsOptional', '@Length', '@MaxLength', '@MinLength', '@Matches',
            '@IsCNPJ', '@IsDocument', '@Transform', '@Get', '@Post', '@Put',
            '@Delete', '@Patch', '@Query', '@Param', '@Body'
        ]
        
        for decorator in decorators:
            if decorator.lower() in line.lower():
                return decorator
        return None

    def _extract_validation_decorators(self, line: str) -> Optional[list]:
        """Extrai decorators de validação da linha"""
        validation_decorators = []
        validation_patterns = [
            '@IsString', '@IsNumber', '@IsBoolean', '@IsArray', '@IsObject',
            '@IsNotEmpty', '@IsOptional', '@Length', '@MaxLength', '@MinLength',
            '@Matches', '@IsCNPJ', '@IsDocument', '@Transform'
        ]
        
        for pattern in validation_patterns:
            if pattern.lower() in line.lower():
                validation_decorators.append(pattern)
        
        return validation_decorators if validation_decorators else None

    def _extract_property_type(self, line: str) -> Optional[str]:
        """Extrai o tipo da propriedade da linha"""
        type_patterns = [
            (r':\s*string\s*[\[\]]?', 'string'),
            (r':\s*number\s*[\[\]]?', 'number'),
    ...
    def _extract_module_name(self, file_path: str) -> Optional[str]:
        ...
        for i, part in enumerate(path_parts):
            if part in ['src', 'lib', 'app'] and i + 1 < len(path_parts):
                return path_parts[i + 1]
        return None

    def find_nest_specific_patterns(self, content: str, file_path: str) -> List[CNPJFieldInterface]:
        ...
        for pattern in dto_patterns:
            ...
            for match in matches:
                ...
                if 'cnpj' in dto_name.lower():
                    fields.append(self.create_cnpj_field(
                        file_path=file_path,
            ...
            for match in matches:
                ...
                if 'cnpj' in property_name.lower():
                    fields.append(self.create_cnpj_field(
                        ...
                        action_needed='Revisar validação de CNPJ',
                        estimated_effort='2-4 horas',
                        nest_type='NESTJS_PROPERTY',
                        property_type='string',
                        validation_decorators=['@IsString', '@IsNotEmpty']
                    ))
