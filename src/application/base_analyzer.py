...
class BaseAnalyzer(ABC):
    ...
    def __init__(self, project_type: str, cnpj_field_class: Type[CNPJFieldInterface] = CNPJFieldBase):
        self.project_type = project_type
        self.cnpj_field_class = cnpj_field_class
        self.console = Console()
        self.logger = logging.getLogger(f"{__name__}.{project_type}")
        self.type_extractor_factory = TypeExtractorFactory()
    ...
    def _analyze_cnpj_field(self, file_path: str, line_num: int, line: str) -> Optional[CNPJFieldInterface]:
    ...
    def create_cnpj_field(self, **kwargs) -> CNPJFieldInterface:
        """Cria uma instância de CNPJField usando a classe específica do analisador"""
        return self.cnpj_field_class(**kwargs)

    def _extract_field_type_and_size(self, line: str, file_path: str = "") -> tuple:
        """
        Extrai tipo e tamanho do campo da linha usando o extractor apropriado
        
        Args:
            line: Linha do código
            file_path: Caminho do arquivo (opcional, usado para determinar o extractor)
            
        Returns:
            Tuple com (tipo, tamanho)
        """
        if file_path:
            # Usar o extractor específico para o tipo de arquivo
            extractor = self.type_extractor_factory.get_extractor(file_path)
            return extractor.extract_type_and_size(line)
        else:
            # Fallback para o método antigo se não tiver file_path
